...
class FeatureExtractor(object):
    ...
    def __init__(self, word_vocab_file, pos_vocab_file):
        ...
        self.transition_id = {('shift', None):0}
        countpair = 0
        self.uniquepairs = [('shift', None)]
        for move in ['right_arc', 'left_arc']:
            for rel in dep_relations:
                countpair+=1
                self.uniquepairs.append((move, rel))
                self.transition_id[(move, rel)] = countpair
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for w in output:
            if w in self.word_vocab:
                output_id.append(self.word_vocab[w])
            else:
                output_id.append(self.word_vocab['<UNK>'])

        return np.array(output_id)
        # return np.zeros(6)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        y = self.transition_id[output_pair]
        return keras.utils.to_categorical(y, num_classes=91)
...
if __name__ == "__main__":
    ...
    np.save('data/uniquepairs.npy', extractor.uniquepairs)
    # print(np.load('data/uniquepairs.npy'))
